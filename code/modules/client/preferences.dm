...
//						age = max(min( round(text2num(new_age)), AGE_MAX),AGE_MIN)
				...
				if("scale_effect")
					var/new_size_mult = input(user, "Choose your character's size ranging from -5 to +9 from normal sprite size. Going over +9 will default to tallest measurment (+25). Note that zero equals roughly 1.66 meters / 5'10", "Character Preference", features["scale_effect"]) as num|null
