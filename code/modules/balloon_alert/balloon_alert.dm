 ...
 */
/atom/proc/balloon_alert(mob/viewer, text, x_offset)
	SHOULD_NOT_SLEEP(TRUE)

	INVOKE_ASYNC(src, PROC_REF(balloon_alert_perform), viewer, text, x_offset)

/// Create balloon alerts (text that floats up) to everything within range.
/// Will only display to people who can see.
/atom/proc/balloon_alert_to_viewers(message, self_message, vision_distance = DEFAULT_MESSAGE_RANGE, list/ignored_mobs, x_offset)
	SHOULD_NOT_SLEEP(TRUE)
	...
	for (var/mob/hearer in hearers)
		...
		balloon_alert(hearer, (hearer == src && self_message) || message, x_offset)
...
/atom/proc/balloon_alert_perform(mob/viewer, text, x_offset)
