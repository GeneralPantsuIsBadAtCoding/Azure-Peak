...
/datum/special_intent/proc/_draw_grid()
	...
	for(var/turf/T in affected_turfs)
		var/obj/effect/temp_visual/swingdelay/fx = new (T, delay)
...
/datum/special_intent/rend
	name = "Rend"
	desc = "Inflicts massive damage if the location is unarmored. Otherwise very little. Has a high chance to dismember."
	tile_coordinates = list(list(0,0))	//Literally just one tile ahead of us.
...
/datum/special_intent/rend/apply_hit()
	var/force = iparent.wielded ? iparent.force_wielded : iparent.force
	var/zone_default = howner.zone_selected
	var/obj/item/I = iparent
	var/mob/living/carbon/human/H = howner
	for(var/turf/T in affected_turfs)
		for(var/mob/living/carbon/human/HT in T.contents)
			...
			var/dist = get_dist(howner, HT)
			
			var/armor_block = HT.run_armor_check(zone_default, "slash", 0, damage = force, used_weapon = I)
			
			if(HT.apply_damage(force, I.damtype, affecting, armor_block))
				affecting.bodypart_attacked_by(H.used_intent.blade_class, force, H, armor = (armor_block ? armor_block : -1), crit_message = TRUE, weapon = I)
			if(force && !armor_block && dist <= 1)
				I.remove_bintegrity(1)
				if(affecting.dismember(I.damtype, H.used_intent?.blade_class, H, H.zone_selected))
					I.add_mob_blood(H)
...
/datum/special_intent/side_sweep/apply_hit()
	for(var/turf/T in affected_turfs)
		for(var/mob/living/carbon/human/HT in T.contents)
			HT.apply_status_effect(/datum/status_effect/debuff/exposed, eff_dur)
	..()

/datum/special_intent/shin_swipe
	name = "Shin Swipe"
	desc = "A hasty attack at the legs, extending ourselves. Slows down the opponent if hit."
	tile_coordinates = list(list(0,0), list(0,1))	//L shape that hugs our flank.
	post_icon_state = "emote"
	pre_icon_state = "blip"
