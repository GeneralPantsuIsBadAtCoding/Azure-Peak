...
/obj/item/clothing/suit/roguetown/shirt/undershirt/disciple/Initialize(mapload)
	. = ..()
	ADD_TRAIT(src, TRAIT_NODROP, CURSED_ITEM_TRAIT)

/obj/item/clothing/suit/roguetown/shirt/undershirt/disciple/dropped(mob/living/carbon/human/user)
	. = ..()
	if(QDELETED(src))
		return
	qdel(src)
...
/obj/item/clothing/suit/roguetown/shirt/undershirt/disciple/take_damage(damage_amount, damage_type, damage_flag, sound_effect, attack_dir, armor_penetration)
	. = ..()
	if(obj_integrity < max_integrity)
		START_PROCESSING(SSobj, src)
		return

/obj/item/clothing/suit/roguetown/shirt/undershirt/disciple/process()
	if(obj_integrity >= max_integrity) 
		STOP_PROCESSING(SSobj, src)
		src.visible_message(span_notice("The [src] tautens with newfound vigor, before relaxing once more."), vision_distance = 1)
		return
	else if(world.time > src.last_repair + src.repair_time)
		src.last_repair = world.time
		obj_integrity = min(obj_integrity + src.repair_amount, src.max_integrity)
