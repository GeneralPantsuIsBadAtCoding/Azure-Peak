...
/turf/proc/CopyOnTop(turf/copytarget, ignore_bottom = 1, depth = INFINITY, flags = null)
	...
	var/turf/new_turf = copytarget.copyTurf(src, flags)
...
/turf/proc/remove_baseturfs_from_typecache(list/typecache)
	if (islist(baseturfs))
		if(length(baseturfs) > 1)
			var/first_baseturf = baseturfs[1]
			baseturfs = list(first_baseturf)
